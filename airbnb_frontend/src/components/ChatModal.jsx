...
function ChatModal({ booking, onClose, currentUser, mode = "guest" }) {
  ...
  useEffect(() => {
    fetchMessages();
    // Optional: Setup a simple interval polling here if we wanted "real-time" feel without WebSockets
  ...
  const getQueryParam = () => {
    if (mode === "host") return "?as_host=true";
    if (mode === "admin") return "?admin_view=true";
    return "";
  };

  const fetchMessages = async () => {
    try {
      const res = await API.get(`bookings/${booking.id}/messages/${getQueryParam()}`);
      setMessages(res.data);
      setLoading(false);
    } catch (err) {
      console.error("Failed to load messages:", err);
      // Don't spam toast on interval polling error
  ...
  const handleSendMessage = async (e) => {
    ...
    try {
      const res = await API.post(`bookings/${booking.id}/messages/${getQueryParam()}`, { content: newMessage });
  ...
  const isGuest = currentUser?.id === booking.user;
  let otherPartyTitle = "Guest";
  if (mode === "guest") otherPartyTitle = "Host";
  if (mode === "admin") otherPartyTitle = "Chat";
      ...
      <div className="bg-white w-full max-w-lg h-[600px] max-h-[90vh] rounded-[2.5rem] shadow-2xl flex flex-col overflow-hidden animate-in zoom-in-95 duration-200">
        ...
        <div className="flex items-center justify-between p-6 border-b border-slate-100 bg-slate-50/50">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h3 className="font-black text-slate-900 text-lg">Message {otherPartyTitle}</h3>
            ...
            messages.map((msg) => {
              ...
              return (
                <div key={msg.id} className={`flex gap-3 ${isMine ? "flex-row-reverse" : "flex-row"}`}>
                  <div className="w-8 h-8 rounded-full bg-slate-200 flex-shrink-0 overflow-hidden flex items-center justify-center border border-slate-100">
                    {msg.sender.avatar ? (
        ...
        <div className="p-4 border-t border-slate-100 bg-white">
          <form onSubmit={handleSendMessage} className="flex items-center gap-3">
            <input
              ...
              className="flex-1 bg-slate-50 border border-slate-200 rounded-full py-3.5 pl-6 pr-4 text-sm font-bold focus:ring-4 focus:ring-rose-500/10 outline-none transition-all"
            />
            <button
              type="submit"
